...
# ===============================
# pCloud RESOLVER (Kodi-native)
# ===============================
def resolve_pcloud(page_url):
    try:
        html = xbmcvfs.File(page_url).read()
...
def get_tmdb_info(title, year):
    try:
        ...
        data = json.loads(xbmcvfs.File(url).read())
...
try:
    # ---- LOAD JSON (Kodi-safe) ----
    json_text = xbmcvfs.File(JSON_URL).read()
    ...
    for movie in data.get("movies", []):
        ...
        if not stream_url:
            xbmc.log("[MoinuMovies] Stream resolve failed", xbmc.LOGERROR)
            continue
